...
import {
  useTherapistProposals,
  useTherapistProposalServices,
  useUpdateTherapistProposalServices,
} from "@/hooks/useTherapistProposals";
...
export default function TherapistProposalEditPage() {
  const params = useParams();
  const { data: proposals, isLoading } = useTherapistProposals();
  const { data: proposalServices, isLoading: servicesLoading } =
    useTherapistProposalServices(params.id as string);
  const updateServicesMutation = useUpdateTherapistProposalServices();
  const currentProposal = proposals?.find((p) => p.id === params.id);
  const { data: allTherapists = [] } = useTherapists();
  const { profile } = useCurrentUser();

  // Fetch services from database
  const { data: allServices = [], isLoading: dbServicesLoading } =
    useServices();
      ...
      evaluationsA.map((evaluation) => {
        if (evaluation.id === id) {
          ...
          if (field === "code") {
            ...
            if (selectedService) {
              updatedEvaluation.name = selectedService.serviceName;
              updatedEvaluation.sessions = selectedService.sessions;
              updatedEvaluation.cost =
                selectedService.sessions *
                Number(selectedService.costPerSession);
          ...
          if (field === "sessions") {
            ...
            // Recalculate cost based on the service's cost per session
            if (updatedEvaluation.code) {
              const selectedService = serviciosEvaluacionData.find(
                (s) => s.code === updatedEvaluation.code
              );
              if (selectedService) {
                updatedEvaluation.cost =
                  updatedEvaluation.sessions *
                  Number(selectedService.costPerSession);
              }
            }
          }
      ...
      treatmentsA.map((treatment) => {
        if (treatment.id === id) {
          ...
          if (field === "code") {
            ...
            if (selectedService) {
              updatedTreatment.name = selectedService.serviceName;
              updatedTreatment.sessions = selectedService.sessions;
          ...
          if (field === "sessions") {
            ...
            // Recalculate cost based on the service's cost per session
            if (updatedTreatment.code) {
              const selectedService = serviciosTratamientoData.find(
                (s) => s.code === updatedTreatment.code
              );
              if (selectedService) {
                updatedTreatment.cost =
                  updatedTreatment.sessions *
                  Number(selectedService.costPerSession);
              }
            }
          }
          ...
          <div className="space-y-4">
            {services.map((service) => {
              ...
              const availableServices = serviceData.filter(
                (serviceItem) => !selectedCodes.includes(serviceItem.code)
              );
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link href={`/therapist/proposals/preview/${params.id}`}>
              <Button
